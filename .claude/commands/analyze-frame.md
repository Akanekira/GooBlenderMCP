...
3. 按以下模板生成子群组分析（直接输出到对话，**不自动写文件**，仅在无独立文档时附于主 Frame 文档末尾）：
...
完成分析后，**直接写文件，无需询问用户**。输出路径：

```
shader-migration/docs/analysis/Frames/Frame<xxx>_<ModuleName>.md
```

文件名格式：`Frame` + 三位编号（如 `009`）+ `_` + 模块标签（如 `Rim`）。

> 写入完成后，在对话中简短告知文件路径即可，无需再次输出完整文档内容。
...
> 溯源 / 日期 / 相关文件 / 上级架构（blockquote）

---
## 📋 模块概述（表格：节点数 / 子框 / 子群组数 / 运算节点 / 职责说明）
## 🗂️ 节点清单（所有子节点列表）
## 📥 外部输入来源（入边表）
## 📤 外部输出（出边表）
## 🔗 子群组（文档状态 + 功能摘要）
## 📊 计算流程（mermaid flowchart）
## 📦 子框功能说明（子帧表）
## 📌 核心链路说明（文字/代码块梳理主要计算路径）
## 💻 HLSL 等价（完整伪代码）
## 🔗 子群组参考（表格 + 调用树）
## ⚙️ 特化参数（材质参数说明）
## 📌 与其他 Frame 的边界（双向接收/输出表）
## 💡 设计要点（表格）
## 🎮 Unity URP 迁移要点（表格）
## ❓ 待确认（checklist）
```
...
| 输出方式 | **分析完成后自动写文件**，无需用户确认；写入完成后告知路径 |
