...
> 📥 **输入**：`_N.RG`(法线贴图) · `NormalStrength`(Group Input) · `SHADERINFO`(光方向 L) · `TANGENT`(切线 T)
> 📤 **输出**：N / V / L / T / B / NoV / ClampNdotV / NoL_Unsaturate / LoV / NdotH / LdotH / TdotH / BdotH / TdotL / BdotL / TdotV / BdotV → Frame.013、Frame.004、Frame.005、Frame.006、Frame.008、Frame.009、Frame.011
> 🔗 **子群组**：`DecodeNormal`、`Get_NoH_LoH_ToH_BoH`
...
> 📥 **输入**：`_D.RGB`(Albedo) · `_P.RGBA`(Metallic/AO/Smoothness/RampUV) · `MetallicMax` · `SmoothnessMax`(Group Input) · `SHADERINFO`
> 📤 **输出**：albedo / metallic / AO / perceptualRoughness / roughness / roughnessT / roughnessB / diffuseColor / fresnel0 / RampUV → Frame.004、Frame.005、Frame.006、Frame.009
> 🔗 **子群组**：`ComputeDiffuseColor`、`ComputeFresnel0`、`PerceptualSmoothnessToPerceptualRoughness` ×3、`PerceptualRoughnessToRoughness` ×3
...
> 📥 **输入**：`NoL_Unsaturate`(Frame.012) · `CastShadow`(SHADERINFO) · `RampUV`(Frame.013) · `diffuseColor`(Frame.013) · `AO`(Frame.013) · Ramp 参数(Group Input)
> 📤 **输出**：`directLighting_diffuse` → Frame.007 ShadowAdjust
> 🔗 **子群组**：`SigmoidSharp` ×2、`RampSelect`、`directLighting_diffuse`
...
> 📥 **输入**：`NdotH / Abs_NdotL / NdotV / LdotH / TdotH / BdotH / TdotL / BdotL / TdotV / BdotV`(Frame.012) · `roughness / roughnessT / roughnessB / fresnel0`(Frame.013)
> 📤 **输出**：`directLighting_specular` → Frame.007 ShadowAdjust
> 🔗 **子群组**：`DV_SmithJointGGX_Aniso`、`F_Schlick`
...
> 📥 **输入**：`clampedNdotV`(Frame.012) · `perceptualRoughness / fresnel0 / diffuseColor`(Frame.013) · `AmbientLighting`(SHADERINFO) · `AmbientLightColorTint / specularFGD_Strength`(Group Input)
> 📤 **输出**：`indirectDiffuse`(混合.009) / `indirectSpecComp`(混合.011) → Frame.007 ShadowAdjust（汇合节点）
> 🔗 **子群组**：`GetPreIntegratedFGDGGXAndDisneyDiffuse`
...
> 📥 **输入**：`directLighting_diffuse`(Frame.005) · `directLighting_specular`(Frame.004) · `indirectDiffuse / indirectSpecComp`(Frame.006) · `GlobalShadowBrightnessAdjustment`(Group Input)
> 📤 **输出**：shadowAdjusted 合并结果 → Frame.008 ToonFresnel
> 🔗 **子群组**：`SmoothStep`
...
> 📥 **输入**：shadowAdjusted(Frame.007) · `Layer weight Value / Offset / fresnelInsideColor / fresnelOutsideColor / ToonfresnelPow`(Group Input)
> 📤 **输出**：toonFresnel 叠加色 → Frame.009 Rim
> 🔗 **子群组**：`SmoothStep`
...
> 📥 **输入**：toonFresnel(Frame.008) · `N / NoV`(Frame.012) · `albedo`(Frame.013) · `dirLight`(SHADERINFO) · `Rim_width_X/Y / Rim_Color / Rim_ColorStrength / LoV 参数`(Group Input)
> 📤 **输出**：`rimResult` → Frame.010 Emission
> 🔗 **子群组**：`DepthRim`、`FresnelAttenuation`、`VerticalAttenuation`、`Rim_Color`、`DirectionalLightAttenuation`
...
> 📥 **输入**：`rimResult`(Frame.009) · `_E.RGB`(贴图) · `Emission Color`(Group Input)
> 📤 **输出**：emissionResult → Frame.011 ThinFilmFilter
> 🔗 **子群组**：（无）
...
> 📥 **输入**：`emissionResult`(Frame.010) · `N / clampedNdotV`(Frame.012) · `shadowScene`(上游) · `Layer weight Value×2 / RS_Index / RS ColorTint / RS Strength / _M 彩色×2 / RS Model`(Group Input)
> 📤 **输出**：`thinFilmResult` → Frame.014 Alpha
> 🔗 **子群组**：（无）
...
> 📥 **输入**：`thinFilmResult`(Frame.011) · `_D.A`(贴图) · Alpha 参数(Group Input)
> 📤 **输出**：`Result / Debug` → GROUP_OUTPUT
> 🔗 **子群组**：`ShaderOutput`
