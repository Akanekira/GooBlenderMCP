...
> 溯源：`docs/raw_data/calculateAngel_20260302.json` · 30 节点
...
| 📥 输入 | 类型 | 说明 |
|---------|------|------|
| `LightDirection` | Vector | 主光源方向（世界空间，来自几何属性） |
...
| 📤 输出 | 类型 | 说明 |
|---------|------|------|
| `AngleThreshold` | Float | 光源相对头部水平旋转角度（映射到 [0,1] 区间） |
| `Flip threshold` | Float | = `sX`（光源在头部坐标系中的左右分量，用于高光翻转判断） |
