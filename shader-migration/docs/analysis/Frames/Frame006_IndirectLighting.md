...
#### 3.1 为什么是 `1/r − 1` 而不是 `1 − r`

`reflectivity`（r）是 FGD LUT 给出的**单次散射总反射率**——打到表面的光，第一跳能反射出多少比例。没被反射出来的 `(1 − r)` 并没有消失，它继续在微面元间弹射。

`ecFactor = (1 − r) / r = 1/r − 1` 表达的是：**每跑出去一份能量，还有多少份在内部循环**。这是真正需要补偿的比率：

| 粗糙度 | r 估算 | `1 − r`（直觉错误） | `1/r − 1`（正确） |
|--------|--------|---------------------|-------------------|
| 光滑金属 | 0.9 | 0.1 | 0.11 |
| 中等粗糙 | 0.5 | 0.5 | **1.0** |
| 极粗糙 | 0.2 | 0.8 | **4.0** |

`1 − r` 上限为 1，无法体现极粗糙材质大量内部弹射的情况；`1/r − 1` 则随粗糙度升高无界放大，才能正确还原多次弹射累积的能量。

#### 3.2 `energyCompFactor = 1 + F0 × ecFactor` 的推导直觉

展开后可以看到它在做什么：

```
direct specular（单次散射）≈ F0 · r
× energyCompFactor           = 1 + F0 · (1/r − 1)
────────────────────────────────────────────────────
≈ F0 · r  +  F0² · (1 − r)
   ↑               ↑
一次弹射出来     二次弹射出来（F0 再折减一次，因为又经历了一次 Fresnel 反射）
```

- `×1`：保留原直接高光不变
- `× F0 × ecFactor`：追加应在微面元间继续弹射但被忽略的能量，用 F0 加权（每次 Fresnel 反射都折减一次 F0）

结果 `energyCompFactor ≥ 1.0`，粗糙度越高放大越多，防止 direct specular 因只计算单次散射而偏暗。

#### 3.3 两条修正路径的分工

同一个 `ecFactor` 分叉出两条互补的修正，缺一不可：

```
ecFactor
  ├─ × F0 + 1  →  energyCompFactor  →  乘 direct specular   （放大已有的直接高光）
  └─ × specFGD × k  →  indirectSpecComp  →  ADD 到总照明   （以间接高光形式补充弹射能量）
```
