...
| — | `理论背景`（按需） | `📖` | 模块涉及的物理 / 数学背景（如 BRDF 积分、薄膜干涉、能量守恒推导等）；仅当 Frame 实现了非平凡物理模型时添加，否则省略此节 |
...
> **子群组链接规则**：章节 8（子群组参考）及文档内任何子群组表格中，凡子群组在 `sub_groups/` 目录已有对应文档，子群组名必须使用 Markdown 链接格式：
> ```
> [`SubGroupName`](../sub_groups/SubGroupName.md)
> ```
> 适用于 4 列（含 `文档状态` 列）和 5 列（含 `详细文档` 列）两种表格格式。无文档的子群组仍用反引号包裹，不加链接。
...
| `📋` | 模块概述 / 规格摘要 | `## 📋 模块概述` 章节标题 |
| `📖` | 理论背景 | `## 📖 理论背景` 章节标题（Frame 涉及非平凡物理 / 数学模型时添加，否则省略） |
...
> 按需添加。若 Frame 实现了非平凡的物理 / 数学模型（如 BRDF 积分、薄膜干涉、能量守恒推导、球谐展开等），在此说明理论基础。不涉及特殊物理的 Frame 可省略此节。
...
说明预计算（LUT / SH / Split-Sum 等）的动机与精度取舍。

### 3. 本 Frame 的简化假设（如有）

列出相对于理论完整形式所做的工程简化及其视觉影响。
